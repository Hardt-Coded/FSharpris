...
module Helpers =
    ...
    let private unflatten (FlattenFields flattenFields) = 
    ...
    let orOperationOnFlattenFields (FlattenFields a) (FlattenFields b) =
        (a,b) 
        ||> Seq.map2 (fun itema itemb -> if itema <> itemb then 1 else 0)
        |> Seq.toList
        |> FlattenFields

    let checkOverlappingOf2FlattenFields (FlattenFields a) (FlattenFields b) =
        (a,b) 
        ||> Seq.exists2 (fun itema itemb -> itema = itemb)
...
let normalizeCoordinates x y brickwidth brickHeight =
    ...
    let y =
        if y < 1 then 1
        elif y + brickHeight - 1 > fieldHeight then fieldHeight - brickHeight + 1
        else y
    (x,y)
...
let createBrickLayer rotation x y brick =
    ...
    //let lineLength = fun line -> line |> List.filter (fun x -> x =1) |> List.length
    let brickWidth = 
        b 
...
let moveBrick move brickstate =
    match brickstate with
    | NoBrick -> failwith "can not move without brick"        
...
let nextInnerGameMove randomRotation randomBrick randomX gamemodel =
    ...
    | Brick (brick,rotation,x,y,matrix) ->
        let nextMoveDown = brick |> createBrickLayer rotation x (y+1)
        match nextMoveDown |> checkCollision gamemodel.GameField with
        | NoCollision -> {gamemodel with BrickState = nextMoveDown}
        | Collision ->
            match nextMoveDown with
            | NoBrick -> failwith "nope, here should be definetly be a brick!"
